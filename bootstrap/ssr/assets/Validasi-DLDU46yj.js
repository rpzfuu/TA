...
import { R as RequestAPI, _ as _sfc_main$5 } from "./AuthenticatedLayout-tMI_FdMz.js";
import { usePage, Head } from "@inertiajs/vue3";
import { _ as _sfc_main$4 } from "./ModalHistoryTemuan-DD9DMnON.js";
  ...
  setup(__props, { expose: __expose, emit: __emit }) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList((_g = _ctx.temuan) == null ? void 0 : _g.rekomendasi, (rekomendasi, index) => {
        _push(`<div class="flex justify-between p-4 mb-6 border rounded-lg shadow-inner bg-gray-50"><div class="w-3/4 pr-4 text-left"><div><p class="text-sm font-semibold"> Rekomendasi ${ssrInterpolate(index + 1)}: </p><p class="py-1 text-gray-700">${ssrInterpolate(rekomendasi == null ? void 0 : rekomendasi.rekomendasi)}</p></div><br><div><p class="text-sm font-semibold"> Tindak Lanjut ${ssrInterpolate(index + 1)}: </p><p class="py-1 text-gray-700">${ssrInterpolate(rekomendasi == null ? void 0 : rekomendasi.tindak_lanjut)}</p></div></div><div class="flex items-center justify-center w-1/4"><span class="flex items-center p-2 border rounded-full">${ssrInterpolate(rekomendasi == null ? void 0 : rekomendasi.status)}</span></div></div>`);
      });
